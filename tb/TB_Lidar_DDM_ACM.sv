/*
 * Copyright (C) 2026 ETH Zurich, University of Bologna
 * Copyright and related rights are licensed under the Solderpad Hardware
 * License, Version 0.51 (the "License"); you may not use this file except in
 * compliance with the License.  You may obtain a copy of the License at
 * http://solderpad.org/licenses/SHL-0.51. Unless required by applicable law
 * or agreed to in writing, software, hardware and materials distributed under
 * this License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR
 * CONDITIONS OF ANY KIND, either express or implied. See the License for the
 * specific language governing permissions and limitations under the License.
 *
 * Authors:
 *  - Alina Zmeu <alina.zmeu@studio.unibo.it>
 */

`timescale 1ns/1ps

module TB_Lidar_DDM_ACM ();

import load_stim_pkg::*;

logic clk_i, rstn_i, ready_ACM, ready_CCM_i, valid_data_i, testmode_i;
logic valid_channel_DDM, valid_azimuth_DDM;
logic [15:0] azimuth_DDM, distance_DDM;
logic [7:0] data_i;
logic [3:0] id_DDM;
logic signed [17:0] cosa1_o, sina1_o, cosa2_o, sina2_o;

logic valid_fs1_ACM_o, valid_fs2_ACM_o, valid_fs2_CCM_i;


logic [18:0] [7:0] data = {8'hFF, 8'hEE, 8'h0, 8'h7, 8'h3, 8'h99, 8'h0, 8'h9, 8'h69, 8'h0, 8'h4, 8'h40, 8'h0,  8'h9, 8'h6F, 8'h0, 8'h5, 8'h2E, 8'h0 }; 



Lidar_DDM dut_ddm(
.clk_i(clk_i),
.rstn_i(rstn_i),
.ready_ACM_i(ready_ACM),
.ready_CCM_i(ready_CCM_i),
.valid_data_i(valid_data_i),
.data_i(data_i),
.testmode_i(testmode_i),
.valid_channel_DDM_o(valid_channel_DDM),
.valid_azimuth_DDM_o(valid_azimuth_DDM),
.azimuth_DDM_o(azimuth_DDM),
.distance_DDM_o(distance_DDM),
.id_DDM_o(id_DDM)
);

Lidar_ACM dut_acm(
    	.rstn_i(rstn_i),
		.clk_i (clk_i),
		.valid_azimuth_DDM_i(valid_azimuth_DDM),
		.azimuth_DDM_i(azimuth_DDM),
		.valid_fs2_CCM_i(valid_fs2_CCM_i),
		.cosa1_o (cosa1_o),
		.sina1_o(sina1_o),
		.cosa2_o (cosa2_o),
		.sina2_o(sina2_o),
		.valid_fs1_ACM_o(valid_fs1_ACM_o),
		.valid_fs2_ACM_o(valid_fs2_ACM_o),
		.ready_ACM_o (ready_ACM)
);


int length_sim;
int index=0;


// Declare stimuli vectors
logic [7:0] bytes[$];

assign length_sim = $bits(bytes)/8;

// Load stimuli from .txt files
initial begin
	load_bytes("../tb/stimuli/vlp_packet.txt", bytes);
end

initial begin
	clk_i='0;
	while(1) begin
		#10;
		clk_i=~clk_i;
	end
end

initial begin
	rstn_i='0;
	#5;
	rstn_i=1'b1;
	
end


initial begin
testmode_i=1'b1;
ready_CCM_i=1'b0;
valid_data_i=1'b0;
valid_fs2_CCM_i=1'b0;
wait(rstn_i)

while(index<=length_sim) begin
@(posedge clk_i);
valid_data_i=1'b1;
data_i=bytes[index];
index++;
end

$stop;
end

endmodule
