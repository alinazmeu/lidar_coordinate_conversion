...
module Lidar_ACM (
	...
	//Data and control logic with CCM
	output logic signed [17:0] cosa1_o         ,
	output logic signed [17:0] sina1_o		   ,
	output logic signed [17:0] sina2_o		   ,
	output logic signed [17:0] cosa2_o		   ,
	...
	typedef enum logic[2:0] {IDLE, START, COMPUTE1, RESULT1, COMPUTE2, RESULT2, WAIT_FOR_CCM} statetype; 
	statetype cs, ns;
	...
	always_comb begin
		...
		if (error_rx_i) begin
			iter_d=1'b0;
		end else if (cs==COMPUTE1 || cs==COMPUTE2)  begin
			iter_d=iter_q+5'd1;
		end
	end
	...
	always_comb begin 
		azimuth_d=azimuth_q;
		if(valid_azimuth_DDM_i & cs==IDLE)
			azimuth_d=azimuth_DDM_i; //VA BENE COS ? O LO DEVO METTERE COME SEGNALE DI ENABLE SINCRONO DEL FF ?
	end 
	...
	else if (iter_q<=5'd16 && (cs==COMPUTE1 || cs==COMPUTE2)) begin
		shifter=iter_q-5'd1;
